...
export function parseBoilCard(value: string) {
  const re = /^\([0-9]{1,4}[A-L][0-9][X-Z,S]{0,1}\)\([0-9]{1,4}\)\([0-9]{1,5}[/]\)\([0-9]{6}\)$/;
...
export function parseProductCard(value: string) {
  const reGP = /^[0-9]{6}#[\w,/,\d,.,\s]+#[0-9]{1,4}[A-L][0-9][X-Z,S]{0,1}#\d{2}.\d{2}.\d{4}#\d{1,3}#\d+$/;
  const reNZ = /^[0-9]{6}#NZ#[0-9]{1,4}[A-L][0-9][X-Z,S]{0,1}#\d{2}.\d{2}.\d{4}#\d+$/;
  const reCode = /\d+(?=#)/;
  const reBatch = /(?<=#)[0-9]{1,4}[A-L][0-9][X-Z,S]{0,1}(?=#)/;
