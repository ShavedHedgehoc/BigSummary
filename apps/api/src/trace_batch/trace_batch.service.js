...
exports.TraceBatchService = void 0;
...
const trace_bt_product_model_1 = __importDefault(require("../../../../../../../src/trace_models/trace_bt_product.model"));
const trace_product_model_1 = __importDefault(require("../../../../../../../src/trace_models/trace_product.model"));
let TraceBatchService = class TraceBatchService {
    constructor(traceBatchRepository) {
        this.traceBatchRepository = traceBatchRepository;
    }
    async getByName(batchName) {
        const traceBatch = await this.traceBatchRepository.findOne({
            where: { BatchName: batchName },
        });
        if (!traceBatch) {
            throw new common_1.HttpException("Партия не найдена", common_1.HttpStatus.NOT_FOUND);
        }
        return traceBatch;
    }
    async getById(id) {
        const traceBatch = await this.traceBatchRepository.findOne({
            attributes: [
                [(0, sequelize_2.col)("TraceBatch.BatchPK"), "batch_id"],
                [(0, sequelize_2.col)("BatchName"), "batch_name"],
                [(0, sequelize_2.col)("BatchDate"), "date"],
                [
                    (0, sequelize_2.literal)("CASE WHEN Plant = 'П' THEN 'Пискаревка' WHEN Plant = 'К' THEN 'Колпино' ELSE '' END"),
                    "plant",
                ],
                [(0, sequelize_2.col)("bt_products.trace_product.ProductId"), "product_id"],
                [(0, sequelize_2.col)("bt_products.trace_product.ProductMarking"), "marking"],
            ],
            where: { BatchPK: id },
            include: [
                {
                    model: trace_bt_product_model_1.default,
                    as: "bt_products",
                    attributes: [],
                    include: [{ model: trace_product_model_1.default, as: "trace_product" }],
                },
            ],
        });
        if (!traceBatch) {
    ...
    OPTION (RECOMPILE)    
    `;
        const countResp = await this.traceBatchRepository.sequelize.query(new_count_query, {
            replacements: {
    ...
    `;
        const countResp = await this.traceBatchRepository.sequelize.query(c_qry2, {
            replacements: {
        ...
        const rowsResp = await this.traceBatchRepository.sequelize.query(dto.filter.sortByBatch
    ...
    async getBatchsWghtReportDetail(dto) {
        const detail_query = `
            SELECT
    ...
    async deleteWeightingsByContainerId(conatinerId) {
        const load_qry = `
        SELECT
        COUNT(*) as count
        FROM
        ...
        ContainerPK = (:ContainerPK)
    `;
        const countResp = await this.traceBatchRepository.sequelize.query(load_qry, {
            replacements: {
        ...
        if (countResp[0].count > 0) {
            throw new common_1.HttpException(`Емкость загружена, удаление невозможно...`, common_1.HttpStatus.BAD_REQUEST);
        }
        await this.traceBatchRepository.sequelize.query(deleteQuery, {
            replacements: {
                ContainerPK: conatinerId,
            },
            type: sequelize_2.default.QueryTypes.DELETE,
        });
    }
    async getWeightingDepartmentSummary(dto) {
        const new_query = `
    WITH Docs AS (
    SELECT 
    d.DocumentPK, 
    ...
    async getWeightingsDepartmentsSummaryDetail(dto) {
        const count_qry_new = `
    SELECT COUNT(*) AS count 
    FROM Weightings w 
    WHERE EXISTS ( 
    ...
    FETCH NEXT (:limit) ROWS ONLY
    `;
        const countResp = await this.traceBatchRepository.sequelize.query(count_qry_new, {
            replacements: {
...
exports.TraceBatchService = TraceBatchService;
exports.TraceBatchService = TraceBatchService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, sequelize_1.InjectModel)(trace_batch_model_1.default, "trace_connection")),
    __metadata("design:paramtypes", [Object])
], TraceBatchService);
//# sourceMappingURL=trace_batch.service.js.map