"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RegulationsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const regulations_service_1 = require("./regulations.service");
let RegulationsController = class RegulationsController {
    constructor(regulationsService) {
        this.regulationsService = regulationsService;
    }
    bulkUpsert(dto) {
        return this.regulationsService.bulkUpdateRegulations(dto);
    }
    getRecordsById(id) {
        return this.regulationsService.getByProductCode(id);
    }
};
exports.RegulationsController = RegulationsController;
__decorate([
    (0, swagger_1.ApiOperation)({ summary: "Создание нового документа сводки" }),
    (0, swagger_1.ApiResponse)({ status: 201 }),
    (0, common_1.Post)("/bulkupsert"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Array]),
    __metadata("design:returntype", void 0)
], RegulationsController.prototype, "bulkUpsert", null);
__decorate([
    (0, swagger_1.ApiOperation)({
        summary: "Получить строку регламента с деталями по id продукта",
    }),
    (0, swagger_1.ApiResponse)({ status: 200 }),
    (0, common_1.Get)("/:id"),
    __param(0, (0, common_1.Param)("id")),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], RegulationsController.prototype, "getRecordsById", null);
exports.RegulationsController = RegulationsController = __decorate([
    (0, swagger_1.ApiTags)("Регламент"),
    (0, common_1.Controller)("regulations"),
    __metadata("design:paramtypes", [regulations_service_1.RegulationsService])
], RegulationsController);
//# sourceMappingURL=regulations.controller.js.map