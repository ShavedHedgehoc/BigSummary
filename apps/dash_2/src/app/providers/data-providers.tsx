...
//     const vanillaClient = createTRPCUntypedClient<AppRouter>({
...
export function DataProvider({ children }: { children: ReactNode }) {
  // 1. Инициализируем только базовый QueryClient
  const [queryClient] = useState(() => new QueryClient());

  // 2. Инициализируем чистый клиент tRPC без перехватчиков сети
  const [trpcClient] = useState(() =>
    trpc.createClient({
      links: [
        httpBatchLink<AppRouterWithTransformer>({
          url: '/trpc_api',
          transformer: superjson, // Сохраняем superjson для работы с объектами Date
        }),
      ],
    })
  );
