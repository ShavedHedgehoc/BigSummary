...
export default function SealantOperationsContent({ summaryData }: { summaryData: ISummary | null }) {
  const { listData, selectedOperation, setSelectedOperation } = useSealantOperationsContent({
      ...
      <VStack h="full" gap={16}>
        ...
        <Text color="fg.subtle" textStyle="xl">
          {selectedOperation.length
            ? `Выбранная операция: ${
                summaryData.sealantOperations.filter((x) => x.id === Number(selectedOperation[0]))[0].description
              }`
            : "Выберите операцию"}
        </Text>
        <OperationsListbox />
