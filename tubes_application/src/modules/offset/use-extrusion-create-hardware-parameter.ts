...
export function useExtrusionCreateHardwareParameter() {
  // const client = useQueryClient();

  const { mutate: createExtrusionHardwareParameter, isPending } = useMutation({
    mutationFn: ExtrusionService.createHardwareCurrentParamsRecord,
    onSuccess: () => {
  ...
  return { createExtrusionHardwareParameter, isPending };
}
