...
import type { DataFormField } from "./data-form-field";
import { useExtrusionInputCurrentParametersStore } from "../extrusion/store/use-extrusion-input-current-parameters-store";
...
export default function RadioEntryModal(props: RadioEntryModalProps) {
  const options = useExtrusionInputCurrentParametersStore(useShallow((state) => state.rondelTypeSelectorOptions));
  return (
    <Dialog.Root
      open={props.open}
            ...
            <Dialog.Body backgroundColor="bg.panel" rounded="lg">
              <RadioCard.Root
                ...
                onValueChange={(e) =>
                  ...
                  props.changeData({
                    key: props.id,
                    // value: options.filter((x) => x.value === e.value)[0].id.toString(),
                    value: e.value,
                  })
                }
              >
