...
export default function ExtrusionInputParametersModal({
  summary_id,
  production_id,
}: {
  ...
  const open = useExtrusionInputParametersModalStore(useShallow((state) => state.open));
  const setOpen = useExtrusionInputParametersModalStore(useShallow((state) => state.setOpen));
  ...
  const handleSaveClick = () => {
    if (summary_id && data && employee) {
      const dto: CreateExtrusionHardwareParamsRecordDto = {
      ...
      setOpen(false);
      createExtrusionHardwareParameter(dto);
      initData();
    }
  };
  ...
  const popoverProps: Partial<InputWithPopoverNumpadProps> &
