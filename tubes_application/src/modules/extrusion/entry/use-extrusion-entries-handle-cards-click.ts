...
export default function useExtrusionEntriesHandleCardsClick() {
  ...
  const handleRadioCardClick = ({ id, title, expectedValue }: { id: string; title: string; expectedValue:string|null|undefined }) => {
    setRadioKey(id);
    setRadioTitle(title);
    setRadioExpectedValue(expectedValue)
    setRadioOpen(true);
  };
  return { handleCardClick, handleBooleanCardClick, handleRadioCardClick };
}
